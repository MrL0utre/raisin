 ...
 *         definitions for red-black hypergraph partitioning algorithms.    
 */
#include "fm.h"
     ...
     INT          k) 
{
    ...
    INT min_crit_path = compute_partition_criticality(h, a, out_neighbors, in_neighbors, sort, partitionp);
    ...
    INT max_crit_path = compute_partition_criticality(h, a, out_neighbors, in_neighbors, sort, partitionp);
    
    INT cur_crit_path = compute_partition_criticality(h, a, out_neighbors, in_neighbors, sort, partition);
                    ...
                    if(!is_computed[partition[v]]) 
                      {
                        ...
                        gain = cur_crit_path - compute_partition_criticality(h, a, out_neighbors, in_neighbors, sort, partition);
    ...
    for(INT i = 0; i < N_MOVES; i++) 
      {    
                  ...
                  if (!is_connected) 
                    {
                      ...
                      pqueue_sift_up(partition_moves[partition[u]], partition_gains[partition[u]], v, partition_gain_sizes[partition[u]]);
                  ...
                  if (partition[v] != selected_part) 
                    {
                      ...
                      partition_gains[selected_part][v] = cur_crit_path - compute_partition_criticality(h, a, out_neighbors, in_neighbors, sort, partition);
                                        
                      pqueue_sift_up(partition_moves[selected_part], partition_gains[selected_part], v, partition_gain_sizes[selected_part]);
       ...
       cur_crit_path = compute_partition_criticality(h, a, out_neighbors, in_neighbors, sort, partition);
       exact = true;
       crit_path_up = 0;
    ...
    INT crit_pathp  = compute_partition_criticality(h, a, out_neighbors, in_neighbors, sort, partitionp);
    INT excrit_path = compute_partition_criticality(h, a, out_neighbors, in_neighbors, sort, ex_partition);
      ...
      {          
    	...
    	for(INT p = 0; p < k; p++) 
         ...
         for(INT wi = 0; wi < nwv; wi++)
          ...
          for(INT x = 0; (x < out_neighbors[u]->size) && (cost[u] <= 0); x++) 
            {              
              v = cell->i;
...
/**
 * @brief compute the critical path according 
  ...
  for(INT i = 0; i < nv; i++)
    {
      ...
      if(is_red[v]) 
        {
          List * cell_out_neighbors = out_neighbors[v];
         
          for(INT x = 0; x < out_neighbors[v]->size; x++) 
            {
              //selection of vertex v
              INT u = cell_out_neighbors->i;
          
              if(partition[u] != partition[v])
                {
                  ncuts++;
                  delays[u] = MAX(delays[u], 
                                  h->ti_delays[v] + h->ti_delays[u] + a->ti_delay[partition[u] * a->i_m + partition[v]]);
                } 
              else 
                {
                  delays[u] = MAX(delays[u], 
                                  h->ti_delays[v] + h->ti_delays[u]);
                }
              cell_out_neighbors = cell_out_neighbors->next;
            }
        }
      else 
        {
              ...
              if(partition[u] != partition[v])
                {
                  ncuts++;
                  delays[u] = MAX(delays[u], 
                                  delays[v] + h->ti_delays[u] + a->ti_delay[partition[u] * a->i_m + partition[v]]);
                } 
              else 
                {
    ...
    INT          k) 
{
  ...
  for(INT i = 0; i < nv; i++) 
    ...
    /* data structure declaration for gains */
