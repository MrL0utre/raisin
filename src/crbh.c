...
void quickSort(int array[], int keys[], int low, int high) {
  if (low < high) {
    ...
    quickSort(array, keys, low, pi - 1);

    // recursive call on the right of pivot
    quickSort(array, keys, pi + 1, high);
  }
}

/* compute the critical path according to a clustering (partition) */
...
int heavyEdgeMatching(Hypergraph * h1, Hypergraph * h2, List ** neighbors, List ** in_neighbors, INT * map, INT k, INT epsilon) {
  ...
  INT nvp    = 0;
  INT nep    = 0;
  INT nv     = h1->i_vertices;
  INT ne     = h1->i_hyperedges;
  INT nr     = h1->i_reds;
  INT np     = h1->i_pins;
  INT nwv    = h1->i_weights;
  INT * reds = h1->ti_reds;
  ...
  // create R star relation
  List ** r_star = (List**)malloc(sizeof(List*)*nv);
  MEM_ERROR(r_star);
  for(INT i = 0; i < nv; i++) {
    r_star[i] = (List*)malloc(sizeof(List));
    MEM_ERROR(r_star[i]);
    new_list(r_star[i]);
    while (r_star[i]->size < in_neighbors[i]->size) {
        list_add_element(r_star[i], 0);
    }
  }
    ...
    for(INT u = 0; u < nv; u++) {
        ...
        cell_in_neighbors = in_neighbors[u];
        cell_r_star       = r_star[u];
        for(INT x = 0; x < in_neighbors[u]->size; x++) {
            //selection of vertex v
            INT v     = cell_in_neighbors->i;
            INT r_val = cell_r_star->i;
            cell_r_star->i = MAX(h1->ti_criticalities_right[v] - (w_hat - h1->ti_criticalities_left[v]), r_val);
            if (is_red[v]||is_red[u]) {
                cell_r_star->i = MAX(h1->ti_criticalities_right[v], w_hat); // arc entrant critique
            }
            cell_in_neighbors = cell_in_neighbors->next;
            cell_r_star = cell_r_star->next;
        }
    }
    ...
    quickSort(h1->ti_criticalities_right, sorted_by_criticality, 0, nv-1);
    ...
    for(INT i = 0; i < nv; i++) {
        ...
        if(!locked[u]) {
            ...
            for(INT x = 0; x < in_neighbors[u]->size; x++) {
                ...
                if(!locked[v] && correct_size && r_v > crit_candidate) {
                    crit_candidate = r_v;
                    candidate      = v;
                } else if (!locked[v] && correct_size && r_v >= crit_candidate && (rand() % 2)>=1 ) {
                    crit_candidate = r_v;
                    candidate      = v;
                }
                cell        = cell->next;
...
/* Compute a subhypergraph h2 of h1 by vertex subset cluster */
int compute_subhypergraph(Hypergraph * h1, Hypergraph * h2, INT * cluster, INT nvp) {
...
/* Compute a reduced hypergraph h2 according to clustering  */
...
int compute_hypergraph_clustering(Hypergraph * h1, Hypergraph * h2, INT * cluster, INT nvp) {
    /* this function only work for matching (cluster of size 2 or 1) */
...
INT           relax) {
  ...
  INT * weights = (INT*)malloc(sizeof(INT)*nv*nwv);
  MEM_ERROR(weights);
  ...
  for(INT u =0; u < nv; u++) {
    ...
    for(INT o = 0; o < neighbors[u]->size; o++) {
        ...
        fusionable = true;
        for(INT wi = 0; wi < nwv; wi++) {
            if(sizes[best_map[u]*nwv+wi]+ sizes[best_map[x]*nwv+wi]>limit[wi]) {
                fusionable = false;
            }
        }
        if(fusionable) {
  ...
  //compute_hypergraph_kclustering(h1, h2, neighbors, in_neighbors, sort, umap, cluster_number);
