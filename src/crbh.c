 ...
 * @brief  Clustering functions declarations for red-black hypergraph
                    ...
                    INT           epsilon) 
{
  ...
  // Array of bool : True iff the index correspond to a red vertices.
  bool * is_red = (bool *) malloc(sizeof(bool) * nv);
  MEM_ERROR(is_red);

  // Initialization of each element of is_red array to false. 
  for (INT i = 0; i < nv; i++) 
    {
      is_red[i] = false;
    }
        ...
        for (INT x = 0; x < in_neighbors[u]->size; x++) 
          {
            ...
            if (is_red[v] || is_red[u]) 
              {
                cell_r_star->i = MAX(h1->ti_criticalities_right[v], w_hat); // incoming critical arc.
              }
