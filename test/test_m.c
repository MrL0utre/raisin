...
int main(void){
    ...
    for(INT i = 1; i < levels; i++) {
     ...
     //derivedBreadthFirstSearchMultilevel(hypergraphs[levels-1], neighbors_list, in_neighbors_list, partition, k, epsilon);
    
    derivedDepthFirstSearchMultilevel(hypergraphs[levels-1], neighbors_list, in_neighbors_list, partition, k, epsilon);
    ...
    topologicalSort(hypergraphs[levels-1], neighbors_list, in_neighbors_list, sort);
    ...
    for(INT i = levels-1; i > 0; i--) {
        ...
        computeListNeighborsUnalloc(hypergraphs[i-1], neighbors_list);
        computeListInNeighborsUnalloc(hypergraphs[i-1], neighbors_list, in_neighbors_list);
        
        topologicalSort(hypergraphs[i-1], neighbors_list, in_neighbors_list, sort);
    ...
    for(INT p = 0; p < k; p++) {
     ...
     multilevel_cut(h, a, neighbors_list, in_neighbors_list, sort, partition, perform, tolerance, k, epsilon);	
     
      cut_cost = compute_partition_cut(h, a, neighbors_list, in_neighbors_list, sort, partition, lambda, k);
      
      printf("final multilevel function cut cost = %d\n", cut_cost);
      
      multilevel_pmax(h, a, neighbors_list, in_neighbors_list, sort, partition_crit, perform, tolerance, k, epsilon);
